...
import { usePairData } from '../../../src/contexts/PairData'
import Link, { CustomLink } from '../Link'
import { Divider } from '../../components'
import { withRouter } from 'react-router-dom'
import { formattedNum, getSwapLink, getPoolLink } from '../../utils'
...
function PairList({ pairs, color, disbaleLinks, maxItems = 10, useTracked = false }) {
  ...
  const ListItem = ({ pairAddress, index }) => {
    const { token0, token1 } = usePairData(pairAddress)
          ...
          {!below1080 && (
            <DataText area="apy">
              <Link external href={getPoolLink(token0?.id, token1?.id)}>
                <ButtonLight color={'#40aeb8'}>+ Add Liquidity</ButtonLight>
              </Link>
              <Link external href={getSwapLink(token0?.id, token1?.id)}>
                <ButtonDark ml={!below1080 && '.5rem'} mr={below1080 && '.5rem'} color={'#40aeb8'}>
                  Trade
                </ButtonDark>
              </Link>
            </DataText>
  ...
  return (
    <ListWrapper>
      ...
      >
        ...
        {!below1080 && <Flex alignItems="center" justifyContent="flexEnd"></Flex>}
      </DashGrid>
      <Divider />
